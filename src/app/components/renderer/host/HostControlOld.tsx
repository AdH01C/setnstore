...
const Path = ({ path, value, updateValue, relations }: any) => {
  ...
  const handlePermissionChange = (method: string, newPermission: any) => {
    ...
    newValue[path].permission[method] =
      newPermission === "{}" ? {} : newPermission;
    updateValue(newValue);
  };

  const dataSource = Object.keys(value[path]?.permission || {}).map(
    (method) => ({
      key: method,
      value: value[path]?.permission?.[method] || null,
      settings:
        value[path]?.permission?.[method] === "Both" ? (
          <div className="flex flex-col items-center gap-2">
            <Select placeholder="Entity" style={{ width: "75%" }}>
              <Option value="jack">Jack</Option>
              <Option value="lucy">Lucy</Option>
              <Option value="tom">Tom</Option>
            </Select>
            <Select placeholder="Permission" style={{ width: "75%" }}>
              <Option value="jack">Jack</Option>
              <Option value="lucy">Lucy</Option>
              <Option value="tom">Tom</Option>
            </Select>
  ...
  const columns = [
    {
      ...
      render: (text: any) => (
        <Input
          ...
          onBlur={(e) => {
            const newValue = { ...value };
            newValue[path].permission[e.target.value] =
              newValue[path].permission[text];
    ...
    <>
      ...
      <div className="flex flex-col w-full items-center justify-between">
          ...
          <Input
            ...
            onChange={(e) => {
              Modal.confirm({
                ...
                content:
                  "Are you sure you want to change the path type? Current path will be deleted and this action cannot be undone.",
                okText: "Yes",
                okType: "primary",
                cancelText: "No",
                onOk() {
        ...
        <div className="flex w-full items-center justify-between">
          ...
          >
            {relations.map(
              (relation: string, index: Key | null | undefined) => (
        ...
        <Typography.Text className="self-start">
